(function () {
  ...
  View.prototype.bindReady = function () {
    var that = this;

    this.$el.find(".ready").on("click", function (event) {
      that.startGame();
      $(event.currentTarget).addClass("inactive");
      $(".difficulty").addClass("inactive");
    });

    this.$el.find(".ready").on("mouseenter", function (event) {
      $(event.currentTarget).html("Go").css("left", "630px");
    });

    this.$el.find(".ready").on("mouseleave", function (event) {
      $(event.currentTarget).html("Ready").css("left", "570px");
    });

    function select_diff(event) {
    ...
    this.$el.find(".beginner").on("click", function (event) {
      select_diff(event);
      that.difficulty = 1;
      that.topSpeed = 50;
    });

    this.$el.find(".intermediate").on("click", function (event) {
      select_diff(event);
      that.difficulty = 3;
      that.topSpeed = 40;
    });

    this.$el.find(".expert").on("click", function (event) {
      select_diff(event);
      that.difficulty = 5;
      that.topSpeed = 30;
    });
  ...
  View.prototype.restartGame = function () {
    if ($(".game-over").hasClass("active")) {
      ...
      $(".game-over").on("transitionend", function () {
        ...
        $(".ready").removeClass("inactive");
        $(".difficulty").removeClass("inactive");
    ...
    window.setTimeout(function () {
      ...
      if (this.board.checkValidMove()) {
        ...
        if (this.checkApple(this.snake.nextMove())) {
          ...
          console.log(this.speedInMs)
          if (this.speedInMs > this.topSpeed) {
            //increment speed when apple is eatenå
            this.speedInMs -= this.difficulty;
          }
